# ---------------------------------------------------------------------------
# catalog-pipeline - Demo 2
#
# Mismo patron que la Demo 1, con dos diferencias:
#   - el producto se publica como TERRAFORM_CLOUD (no EXTERNAL), que es el tipo
#     que enruta a las colas ServiceCatalogTerraformCloud* del motor de HashiCorp
#     y al terraform-parameter-parser de ESE motor
#   - el Portfolio no se crea aqui: se reutiliza el que ya crea el motor
#
# GitHub -> Build/Validate -> Publish en Service Catalog.
#
...
locals {
  ...
  portfolio_id    = data.terraform_remote_state.bootstrap.outputs.portfolio_id
  launch_role_arn = data.terraform_remote_state.bootstrap.outputs.launch_role_arn
...
resource "aws_s3_bucket" "artifacts" {
  bucket        = "aurex-sc-artifacts-tfc-${local.account_id}-${local.region}"
  force_destroy = true
}
