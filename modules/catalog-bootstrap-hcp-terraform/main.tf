# ---------------------------------------------------------------------------
# catalog-bootstrap (motor HCP Terraform)
#
# El motor de HashiCorp ya crea su propio "TFC Example Portfolio" al aplicar,
# asi que aqui NO se crea un portfolio nuevo: se reutiliza ese, y este modulo
# se limita a lo que el motor no trae de serie:
#
#   1. Acceso al Portfolio para el principal que va a lanzar el producto
#   2. Un Launch Role para el producto standard-environment que publica la
#      pipeline (el motor solo crea el de SU producto de ejemplo)
# ---------------------------------------------------------------------------
...
locals {
  ...
  parameter_parser_role_arn = var.engine_parameter_parser_role_arn
  send_apply_role_arn       = var.engine_send_apply_role_arn
